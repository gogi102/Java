...
public class BaseBall {
    ...
    public static int playGame() throws IOException{
        ...
        x = Math.abs(r.nextInt()%9)+1; // x를 0~8까지 +1을 해준값을 - 값이 나오지 않게 절대값으로 할당시켜줌
        do { y = Math.abs(r.nextInt()%9)+1; } while (x == y); //y에 값을 할당 시켜주고 x와 y의 값이 같으면 같지 않을 때 까지 할당
        do { z = Math.abs(r.nextInt()%9)+1; } while (x == z || y == z); // z에 값을 할당 시켜주고 x와 z, y와 z의 값이 같으면 같지 않을 때 까지 할당
        return playGame(x,y,z); // 밑에 있는 playGame(x,y,z)를 실행시킨 값을 반환
    }

    public static int playGame(int x, int y, int z) throws IOException{ // playGame메서도 생성 throws IOException는 예외처리를 위해서 선언

        int count, strike, ball;
        int[] usr = new int[3]; // usr이라는 3칸 짜리 배열선언
        int[] com = {x,y,z}; // com 배열에 x,y,z를 할당

        System.out.println("숫자 야구 게임 시작");

        count = 0; // 시도 횟수를 적어줄 count 선언
        ...
        do {
            ...
            do { // 무조건 1번은 실행 후 밑에 while 조건에 따라 결정한다
                ...
                // 사용자에게 입력 받기 위해 BufferedReader 호출(BufferedReader는 InputStreamReader를 입력 받아 문자열로 출력해줌,
                // BufferedReader는 \n이 입력되기 직전까지 받은 모든 텍스트를 저장하고 stream이 다 차거나 null이 아니라면 그 값을 계속 가지고 있음)
                // BufferedReader는 Scanner보다 속도가 빠름 입력이 많을 때도 유리함
                String user;
                ...
                if(usr[0] == 0 || usr[1] == 0 || usr[2] == 0){
                    System.out.println("0은 입력 하지 마세요. 다시 입력해 주세요"); // 0인가?를 판단
                }
                else if(usr[0] > 9 || usr[1] > 9 || usr[2] > 9){
                    System.out.println("1부터 9까지 숫자 중 하나를 입력해 주세요. 다시 입력해 주세요"); // 9를 넘지 않았는가?를 판단
                }
                else if(usr[0] == usr[1] || usr[1] == usr[2] || usr[0] == usr[2]){
                    System.out.println("모두 다른 숫자를 입력해주세요. 다시 입력해 주세요"); // 중복인가?를 판단
                }
